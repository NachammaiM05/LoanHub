				...
				<table class="table table-bordered" id="loan-list">
					...
					<tbody>
						<?php
							...
							$qry = $conn->query("SELECT p.*,l.ref_no,concat(b.firstname,' ',b.lastname,' ')as name, b.mobile_no, b.address from payments p inner join loan_list l on l.id = p.loan_id inner join borrowers b on b.id = l.borrower_id  order by p.id asc");
						 ...
						 <tr>
						 	...
						 	<td class="text-center">
						 			...
						 			<button class="btn btn-outline-danger btn-sm delete_payment" type="button" data-id="<?php echo $row['id'] ?>"><i class="fa fa-trash"></i></button>
						 	             <!-- Redirect link to payment transaction page -->
										  <a href="payment_transaction.php?id=<?php echo $row['id'] ?>" class="btn btn-outline-info btn-sm"><i class="fa fa-info-circle"></i> Transaction Details</a>
								</td>
