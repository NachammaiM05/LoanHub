...
Class Action {
	...
	public function addBorrower($name, $email, $phone) {
		if (!isset($_SESSION['user_id'])) {
			echo json_encode(['error' => 'User not logged in']);
			exit;
		}
		// Proceed with adding borrower
	...
	function login2(){
		extract($_POST);
		$hashed_password = md5($password);  // You can replace this with a more secure password hashing mechanism
	...
	function save_lender() {
		...
		if ($id) {
			// Update lender
			$sql = "UPDATE lenders SET firstname='$firstname', lastname='$lastname', email='$email' WHERE id='$id'";
		} else {
			// Insert new lender
			$sql = "INSERT INTO lenders (firstname, lastname, email) VALUES ('$firstname', '$lastname', '$email')";
		}
		$result = $this->conn->query($sql);
		return $result ? 1 : 0;
	}
	public function editLender($id, $name, $email, $phone) {
		if (!isset($_SESSION['user_id'])) {
			echo json_encode(['error' => 'User not logged in']);
			exit;
		}
